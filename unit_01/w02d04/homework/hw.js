...
const Presenter = {
  insertCounterComponent: function(newCountId){
    ...
    //  My attempt JSQuery
    //  var $newCounterHTML = `<div class='counter' data-index='2'><h3>Count: <span>0</span></h3><button class='increment'> + 1 </button></div>`;
    //  $('#counter-list').append($newCounterHTML);
  ...
  removeCounterComponent: function(countId){            
    ...
    let removeCounterComponent = document.querySelector(`[data-count-id="${countId}"]`);
    removeCounterComponent.parentNode.removeChild(removeCounterComponent);    
  }
};
